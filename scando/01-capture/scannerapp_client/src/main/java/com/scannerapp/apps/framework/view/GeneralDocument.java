    ...
    implements java.io.Serializable
{
      ...
      BadLocationException
  {
    ...
    else
    {
      switch ( textCase )
      {
        case GeneralTextField.AS_TYPED:
        ...
        case GeneralTextField.ALL_UPPER:
          if ( isValidTextType ( aString ) )
          {
        ...
        case GeneralTextField.ALL_LOWER:
          if ( isValidTextType ( aString ) )
          {
  ...
  public boolean isValidTextType ( String aString )
  {
    ...
    switch ( textType )
    {
      ...
      case GeneralTextField.CHARACTERS:
        for ( int i = 0 ; i < cArray.length ; i++ )
        {
          if ( strNUMBERS.indexOf ( cArray[ i ] ) != -1 )
          {
  ...
  public GeneralDocument ( int aMin , int aMax , int aTextCase , int aTextType )
  {
    min = aMin ;
    max = aMax ;
    textCase = aTextCase ;
